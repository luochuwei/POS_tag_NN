...
def get_candidate_dic_for_test_pos(sents, dim, i2w):
    candidate_dic_for_test_pos = {}
    for s in xrange(int(sents.shape[1] / dim)):
        xs = sents[:, s * dim : (s + 1) * dim]
        candidate_dic_for_test_pos[s] = []
        for w_i in xrange(xs.shape[0]):
            w = i2w[np.argmax(xs[w_i, :])]
            if w == "<eoss>":
                break
                ...
                candidate_dic_for_test_pos[s].append(w)
    return candidate_dic_for_test_pos
...
def new_processing(x_path, y_path, x_tag_path, y_tag_path, i2w, w2i, i2t, t2i, get_num_start, get_num_end, batch_size = 1):
    """
    load已有的w2i i2w t2i i2t
    """
    X_seqs = []
    y_seqs = []
    y_tag_seqs = []
    ...
    return X_seqs, y_seqs, y_tag_seqs, i2w, w2i, t2i, i2t, data_x_y




def test_sentence_input_processing_long(sentence, tag_seq, i2w, w2i, i2t, t2i, yl, batch_size):
    x_words = sentence.split() + ["<eoss>"]
    x_tag = tag_seq.split() + ["<eoss>"]
    assert len(x_words) == len(x_tag)
    x = np.zeros((len(x_words), len(w2i)), dtype = theano.config.floatX)
    xt = np.zeros((len(x_tag), len(t2i)), dtype = theano.config.floatX)
    y = np.zeros((yl, len(w2i)), dtype = theano.config.floatX)
    yt = np.zeros((yl, len(t2i)), dtype = theano.config.floatX)
    ...
    for m in xrange(0, len(x_tag)):
        if x_tag[m] in t2i:
            ...
            print "error x tag !"
    X_seqs = [np.column_stack((x, xt)) for i in range(batch_size)]
    Y_seqs = [y for i in range(batch_size)]
    Y_tag_seqs = [yt for i in range(batch_size)]
    data_x_y = batch_sequences(X_seqs, Y_seqs, Y_tag_seqs, i2w, w2i, t2i, i2t, batch_size)
    return data_x_y

def test_processing_long(path, path_pos, i2w, w2i, i2t, t2i, yl, batch_size):
    f = open(path)
    f_tag = open(path_pos)
    X_seqs = []
    Y_seqs = []
    Y_tag_seqs = []
    for line, line_tag in zip(f, f_tag):
        # print i
        x_words = line.strip().split() + ["<eoss>"]
        x_tag = line_tag.strip().split() + ["<eoss>"]
