...
class Seq2Seq(object):
    ...
    def define_layers(self):
        ...
        self.activation_dim_y = word_decoder_layer.activation_dim_y
        self.activation_dim_pos = word_decoder_layer.activation_dim_pos


        self.predict = theano.function(inputs = [self.X, self.mask, self.mask_y, self.batch_size],
                                               givens = {self.is_train : np.cast['int32'](1)},
                                               outputs = [self.activation_dim_y, self.activation_dim_pos],on_unused_input='ignore')
    ...
    def define_train_test_funcs(self):
        ...
        self.train = theano.function(inputs = [self.X, self.Y_y, self.Y_pos, self.mask, self.mask_y, lr, self.batch_size],
                                               givens = {self.is_train : np.cast['int32'](1)},
                                               outputs = [cost, self.activation_dim_y, self.activation_dim_pos],
                                               updates = updates)
